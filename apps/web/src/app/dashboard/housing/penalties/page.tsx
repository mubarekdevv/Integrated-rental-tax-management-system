...
export default async function HousingPenaltiesPage() {
  const [penalties, activeAgreements, penaltyRules] = await Promise.all([
    prisma.penalty.findMany({
      orderBy: { createdAt: "desc" },
      include: { responsibleParty: true, agreement: { include: { property: true } }, penaltyRule: true },
    }),
    prisma.rentalAgreement.findMany({
      where: { status: "ACTIVE" },
      include: { property: { include: { ownerships: { include: { ownerProfile: true } } } } },
    }),
    prisma.penaltyRule.findMany({ where: { isActive: true } }),
  ]);
